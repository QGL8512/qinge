/*
 * @Author: your name
 * @Date: 2020-10-26 16:35:48
 * @LastEditTime: 2020-10-31 22:46:32
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \数据可视化\js\index.js
 */
$(function() {
...
$(function() {
        ...
        let $orders = $('.order h4:eq(0)')
        let $moneys = $('.order h4:eq(1)')
        $('.order').on('click', '.filter a', function(e) {
            $(this).addClass('active').siblings().removeClass('active')
            let idx = $(this).index()
            let { orders, amount } = data[idx]
            $orders.text(orders)
            $moneys.text(amount)
            index = idx
            e.stopPropagation()
...
$(function() {
    ...
    $links.on('click', function() {
        ...
        option.series[0].data = data[date][0]
        option.series[1].data = data[date][1]
        myChart.setOption(option)
    })
...
$(function() {
    ...
    // 步骤：
    //  1. 鼠标移入到导航li上
    //  2. 获取到当前导航li上的data-city的属性值
    //  3. 根据data-city的属性值获取到对应的数据
    //  4. 在根据对应的数据，创建li元素添加到ul.sub中
    let $top = $('.top')
    let $lists = $('.sup li')
    let $sub = $('.sub')
    let count = 0
    ...
    $lists.eq(0).mouseenter()

    function timer() {
        timerId = setInterval(() => {
            count = ++count % $lists.length
            $lists.eq(count).mouseenter()
        }, 1000);
    }
    timer()
